                    ...
                    int var_num, bool terminate)
{
    ...
    local_bb.num_dims = 2;
    memcpy(local_bb.lb.c, lb, 2*sizeof(uint64_t));
    memcpy(local_bb.ub.c, ub, 2*sizeof(uint64_t));

    struct bbox* src_bbox_tab = (struct bbox*) malloc(src_np[0]*src_np[1]*sizeof(struct bbox));
    int iter[2];
    for(iter[0]=0; iter[0]<src_np[0]; iter[0]++) {
        for(iter[1]=0; iter[1]<src_np[1]; iter[1]++) {
                src_bbox_tab[iter[0]*src_np[1]+iter[1]].num_dims = 2;
                for(int d=0; d<2; d++) {
                    ...
                    int var_num, bool terminate)
{
    ...
    ViewHost_t v_G("GetView", sp[0], sp[1]);
    ViewHost_t v_tmp("TmpView", sp[0], sp[1]);
